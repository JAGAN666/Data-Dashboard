  ...
  const fetchCounties = async (stateFips: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error("Failed to fetch counties from Census API")
      }
    } catch (error) {
      ...
      if (stateCode && FALLBACK_COUNTIES[stateCode as keyof typeof FALLBACK_COUNTIES]) {
        ...
        setCounties(FALLBACK_COUNTIES[stateCode as keyof typeof FALLBACK_COUNTIES])
        setSelectedCounty(FALLBACK_COUNTIES[stateCode as keyof typeof FALLBACK_COUNTIES][0].fips)
      } else {
